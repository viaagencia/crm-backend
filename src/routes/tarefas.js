...
router.post('/', async (req, res) => {
  try {
    const { usuarioId, contatoId, titulo, status, dataHora } = req.body;

    if (!usuarioId || !contatoId || !titulo) {
      return res.status(400).json({ erro: 'usuarioId, contatoId e titulo são obrigatórios' });
    }
    ...
    await dbRun(
      `INSERT INTO tarefas (id, usuarioId, contatoId, titulo, status, dataHora, criadoEm)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, usuarioId, contatoId, titulo, status || 'pendente', dataHora || null, agora]
    );
