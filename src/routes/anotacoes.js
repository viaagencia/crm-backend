...
router.post('/', async (req, res) => {
  try {
    const { usuarioId, contatoId, texto } = req.body;

    if (!usuarioId || !contatoId || !texto) {
      return res.status(400).json({ erro: 'usuarioId, contatoId e texto são obrigatórios' });
    }
    ...
    await dbRun(
      `INSERT INTO anotacoes (id, usuarioId, contatoId, texto, criadoEm)
       VALUES (?, ?, ?, ?, ?)`,
      [id, usuarioId, contatoId, texto, agora]
    );
