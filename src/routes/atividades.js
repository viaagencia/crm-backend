...
router.post('/', async (req, res) => {
  try {
    const { usuarioId, contatoId, contatoTipo, tipo, status, observacao } = req.body;

    if (!usuarioId || !contatoId || !tipo || !status) {
      return res.status(400).json({ erro: 'usuarioId, contatoId, tipo e status são obrigatórios' });
    }
    ...
    await dbRun(
      `INSERT INTO atividades (id, usuarioId, contatoId, contatoTipo, tipo, status, observacao, criadoEm)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, usuarioId, contatoId, contatoTipo || 'lead', tipo, status, observacao || '', agora]
    );
