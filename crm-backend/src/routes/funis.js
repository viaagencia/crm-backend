...
router.post('/', async (req, res) => {
  try {
    ...
    if (!nome) {
      return res.status(400).json({ erro: 'Nome do funil é obrigatório' });
    }
    ...
    const countResult = await dbGet('SELECT COUNT(*) as count FROM funis');
    const ordem = (countResult?.count || 0) + 1;

    await dbRun(
      `INSERT INTO funis (id, nome, descricao, ordem, criadoEm, atualizadoEm)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, nome, descricao || '', ordem, agora, agora]
    );
